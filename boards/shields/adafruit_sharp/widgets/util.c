...
void rotate_canvas(lv_obj_t *canvas)
{
    ...
    lv_draw_sw_rotate(buf_copy, buf, CANVAS_SIZE, CANVAS_SIZE, stride, stride,
                      LV_DISPLAY_ROTATION_0, CANVAS_COLOR_FORMAT);
}

void draw_battery(lv_obj_t *canvas, const struct status_state *state)
{
    for (int i = SOURCE_OFFSET; i < ZMK_SPLIT_BLE_PERIPHERAL_COUNT + 1; i++)
    {
        struct battery_state battery_state_status = state->battery_state[i];
        uint8_t x_offset = (i - SOURCE_OFFSET) * 110;
        ...
        canvas_draw_rect(canvas, x_offset + 2, 4, (battery_state_status.battery + 2) / 4, 8, &rect_white_dsc);
        canvas_draw_rect(canvas, x_offset + 30, 5, 3, 6, &rect_white_dsc);
        canvas_draw_rect(canvas, x_offset + 31, 6, 1, 4, &rect_black_dsc);

        if (battery_state_status.charging)
        {
