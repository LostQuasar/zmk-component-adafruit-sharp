...
struct output_status_state
{
    ...
    bool profiles_connected[DISPLAY_PROFILE_COUNT];
    bool profiles_bonded[DISPLAY_PROFILE_COUNT];
};
...
static void draw_top(lv_obj_t *widget, const struct status_state *state)
{
    ...
    canvas_draw_text(canvas, 116, 52, 24, &label_dsc_wpm, wpm_text);
...
static void draw_bottom(lv_obj_t *widget, const struct status_state *state)
{
    ...
    if (state->layer_label == NULL || strlen(state->layer_label) == 0)
    {
        ...
        canvas_draw_text(canvas, 0, 0, 168, &label_dsc, text);
    }
    else
    {
        canvas_draw_text(canvas, 0, 0, 168, &label_dsc, state->layer_label);
    }
...
static struct output_status_state output_status_get_state(const zmk_event_t *_eh)
{
    ...
    for (int i = 0; i < MIN(DISPLAY_PROFILE_COUNT, ZMK_BLE_PROFILE_COUNT); ++i)
    {
